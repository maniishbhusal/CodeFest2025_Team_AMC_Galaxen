  ...
  return (
    <div className="divide-y divide-gray-100">
      {patients.map((patient) => (
        ...
        >
          {/* Risk indicator dot */}
          <div className={`w-3 h-3 rounded-full ${getRiskColor(patient.mchat_risk)}`} />
          ...
          <span className={`text-sm font-medium px-2.5 py-1 rounded-lg ${
            patient.mchat_risk === "high" ? "bg-red-100 text-red-700" :
            patient.mchat_risk === "medium" ? "bg-amber-100 text-amber-700" :
            patient.mchat_risk === "low" ? "bg-green-100 text-green-700" :
            "bg-gray-100 text-gray-600"
          }`}>
            {patient.mchat_risk ? `${patient.mchat_risk.charAt(0).toUpperCase()}${patient.mchat_risk.slice(1)}` : "N/A"}
          </span>
        </div>
      ))}
    </div>
